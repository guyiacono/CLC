...
class SurveyP6: UIViewController {
    ...
    {
        qAnswer![25] = Int(q1custom.value)
        qAnswer![26] = Int(q2custom.value)
        qAnswer![27] = Int(q3custom.value)
        qAnswer![28] = Int(q4custom.value)
        qAnswer![29] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP7", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP7")
        {
            let destinationVC = segue.destination as! SurveyP7
