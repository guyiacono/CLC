...
class SurveyP13: UIViewController {
    ...
    {
        qAnswer![60] = Int(q1custom.value)
        qAnswer![61] = Int(q2custom.value)
        qAnswer![62] = Int(q3custom.value)
        qAnswer![63] = Int(q4custom.value)
        qAnswer![64] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP14", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP14")
        {
            let destinationVC = segue.destination as! SurveyP14
