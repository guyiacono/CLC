...
class SurveyP2: UIViewController {
    ...
    {
        qAnswer![5] = Int(q1custom.value)
        qAnswer![6] = Int(q2custom.value)
        qAnswer![7] = Int(q3custom.value)
        qAnswer![8] = Int(q4custom.value)
        qAnswer![9] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP3", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP3")
        {
            let destinationVC = segue.destination as! SurveyP3
