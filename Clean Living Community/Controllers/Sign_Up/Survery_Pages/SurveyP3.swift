...
class SurveyP3: UIViewController {
    ...
    {
        qAnswer![10] = Int(q1custom.value)
        qAnswer![11] = Int(q2custom.value)
        qAnswer![12] = Int(q3custom.value)
        qAnswer![13] = Int(q4custom.value)
        qAnswer![14] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP4", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP4")
        {
            let destinationVC = segue.destination as! SurveyP4
