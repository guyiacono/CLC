...
class SurveyP11: UIViewController {
    ...
    {
        qAnswer![50] = Int(q1custom.value)
        qAnswer![51] = Int(q2custom.value)
        qAnswer![52] = Int(q3custom.value)
        qAnswer![53] = Int(q4custom.value)
        qAnswer![54] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP12", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP12")
        {
            let destinationVC = segue.destination as! SurveyP12
