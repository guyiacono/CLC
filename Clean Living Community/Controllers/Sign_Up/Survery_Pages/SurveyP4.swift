...
class SurveyP4: UIViewController {
    ...
    {
        qAnswer![15] = Int(q1custom.value)
        qAnswer![16] = Int(q2custom.value)
        qAnswer![17] = Int(q3custom.value)
        qAnswer![18] = Int(q4custom.value)
        qAnswer![19] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP5", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP5")
        {
            let destinationVC = segue.destination as! SurveyP5
