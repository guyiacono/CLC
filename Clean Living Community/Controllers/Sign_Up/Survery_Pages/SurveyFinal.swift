...
class SurveyFinal: UIViewController {
    ...
    */
    ...
    {
        qAnswer[0] = Int(q1custom.value)
        qAnswer[1] = Int(q2custom.value)
        qAnswer[2] = Int(q3custom.value)
        qAnswer[3] = Int(q4custom.value)
        qAnswer[4] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP2", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP2")
        {
            let destinationVC = segue.destination as! SurveyP2
