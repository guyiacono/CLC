...
class SurveyP19: UIViewController {
    ...
    {
        qAnswer![90] = Int(q1custom.value)
        qAnswer![91] = Int(q2custom.value)
        qAnswer![92] = Int(q3custom.value)
        qAnswer![93] = Int(q4custom.value)
        qAnswer![94] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP20", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP20")
        {
            let destinationVC = segue.destination as! SurveyP20
