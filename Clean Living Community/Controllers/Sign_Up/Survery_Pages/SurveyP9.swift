...
class SurveyP9: UIViewController {
    ...
    {
        qAnswer![40] = Int(q1custom.value)
        qAnswer![41] = Int(q2custom.value)
        qAnswer![42] = Int(q3custom.value)
        qAnswer![43] = Int(q4custom.value)
        qAnswer![44] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP10", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP10")
        {
            let destinationVC = segue.destination as! SurveyP10
