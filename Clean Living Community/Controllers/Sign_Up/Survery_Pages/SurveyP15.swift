...
class SurveyP15: UIViewController {
    ...
    {
        qAnswer![70] = Int(q1custom.value)
        qAnswer![71] = Int(q2custom.value)
        qAnswer![72] = Int(q3custom.value)
        qAnswer![73] = Int(q4custom.value)
        qAnswer![74] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP16", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP16")
        {
            let destinationVC = segue.destination as! SurveyP16
