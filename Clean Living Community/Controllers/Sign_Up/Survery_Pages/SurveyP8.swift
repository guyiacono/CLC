...
class SurveyP8: UIViewController {
    ...
    {
        qAnswer![35] = Int(q1custom.value)
        qAnswer![36] = Int(q2custom.value)
        qAnswer![37] = Int(q3custom.value)
        qAnswer![38] = Int(q4custom.value)
        qAnswer![39] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP9", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP9")
        {
            let destinationVC = segue.destination as! SurveyP9
