...
class SurveyP14: UIViewController {
    ...
    {
        qAnswer![65] = Int(q1custom.value)
        qAnswer![66] = Int(q2custom.value)
        qAnswer![67] = Int(q3custom.value)
        qAnswer![68] = Int(q4custom.value)
        qAnswer![69] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP15", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP15")
        {
            let destinationVC = segue.destination as! SurveyP15
