...
class SurveyP18: UIViewController {
    ...
    {
        qAnswer![85] = Int(q1custom.value)
        qAnswer![86] = Int(q2custom.value)
        qAnswer![87] = Int(q3custom.value)
        qAnswer![88] = Int(q4custom.value)
        qAnswer![89] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP19", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP19")
        {
            let destinationVC = segue.destination as! SurveyP19
