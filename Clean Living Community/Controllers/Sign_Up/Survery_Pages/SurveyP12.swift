...
class SurveyP12: UIViewController {
    ...
    {
        qAnswer![55] = Int(q1custom.value)
        qAnswer![56] = Int(q2custom.value)
        qAnswer![57] = Int(q3custom.value)
        qAnswer![58] = Int(q4custom.value)
        qAnswer![59] = Int(q5custom.value)
        performSegue(withIdentifier: "toSurveyP13", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if(segue.identifier == "toSurveyP13")
        {
            let destinationVC = segue.destination as! SurveyP13
